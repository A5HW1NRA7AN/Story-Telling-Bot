...
# --- Streamlit App Title ---
st.title("Story Teller, Fairy Tale Story Generator")
st.header("Using Ollama Models and RAG by ChromaDB")

# --- Sidebar Controls ---
st.sidebar.header("ModelSettings")
...
# --- User Input ---
st.header("What Kind of Story do you want to hear?")
user_query = st.text_input("Enter your fantasies here:", value="")

# --- Load Data and Initialize ChromaDB ---
@st.cache_resource
def load_chromadb_collection():
    client = chromadb.PersistentClient(r"C:\Users\rajan\OneDrive\Desktop\College\LLM\Stroy-Generation-using-multiple-models\Stroy-Generation-using-multiple-models")
...
@st.cache_data
def load_text_data():
    df = pd.read_csv(r"C:\Users\rajan\OneDrive\Desktop\College\LLM\Stroy-Generation-using-multiple-models\Stroy-Generation-using-multiple-models\data\1000Folk_Story_around_the_Globe.csv")
...
if st.button("Get Stories"):
    with st.spinner("Retrieving similar stories..."):
        ...
        st.subheader("Similar Stories if you are interested")
    ...
    with st.spinner(f"Generating a new story using {selected_model}..."):
        ...
        if ollama_response:
            st.subheader("Your Story is Here")
